...
(library (ev syntax)
  ...
  (define syntax->function-name-string
    (lambda (stx)
      (datum->syntax
        stx
        (string-map (lambda (c)
  ...
  (define string-filter
    (lambda (func str)
      (list->string
        (reverse
          (fold-left func '() (string->list str))))))

  (define has-ev-tstamp?
    (lambda (syms)
      (find
        (lambda (s)
          (eq? s 'ev-tstamp))
...
(library (ev)
  ...
  (define load-libev-ffi
    (load-shared-object (locate-library-object "ev-ffi.so")))
